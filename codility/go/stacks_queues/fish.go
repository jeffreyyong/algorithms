...
fish P and Q meet each other when P < Q, B[P] = 1 and B[Q] = 0, and there are no living fish between them. After they meet:
...
// Check if fish is downstream and add to stack. If fish is upstream check how many it will eat from downstream
func fishAlive(A, B []int) int {
	l := NewIntStack(len(A))
	response := 0
	...
	for i, _ := range B {
		if B[i] == 1 {
			...
			for l.size > 0 {
				if l.Front() < A[i] { // We have a matching pair
					l.Pop()
				} else {
					break
				}
