'''
Find the length of the longest substring T fo a given string (consists of lowercase letters
only) such that every character in T appears no less htan k times

Example 1:
s = "aaabb", k = 3

Output:
3

The longest substring is "aaa", as :a: is repeated 3 times

Example 2:
Input:
    s = "ababbc", k = 2

Output:
    5

The longest substring is "ababb", as 'a' is repeated 2 times and 'b' is repeated 3 times.
'''


'''
Intuition:

If every character appears at least k times, the whole string is ok. Otherwise split by a least frequent
character ( because it will always be too infrequent and thus can't be part of any ok substring) and 
make the most out of the splits.

'''

class Solution:

    def longest_substring(self, s, k):

        for c in set(s):
            if s.count(c) < k:
                return max(self.longest_substring(t, k) for t in s.split(c))

        return len(s)
