...
func commonCharacter(words []string, curWord string, charToCheck byte) string {
	if len(words) == 0 {
		return ""
	}
...
func getMaxPartition(c byte, words []string) (bool, []string) {
	...
	maxPartitionKey := string(emptyKey)
	maxPartitionLen := len(partitions[maxPartitionKey])
	// fmt.Println("maxPartitionLen", maxPartitionLen)
	// fmt.Println("partitions", partitions)
	for k, partition := range partitions {
		if len(partition) > maxPartitionLen {
			// fmt.Println("partitionLen", len(partition))
			// fmt.Println("maxPartitionLen", maxPartitionLen)
			maxPartitionLen = len(partition)
			maxPartitionKey = k
			// fmt.Println("maxPartitionKey", maxPartitionKey)
		}
	}

	return (maxPartitionKey != string(emptyKey)), partitions[maxPartitionKey]
...
func runUserInputLoop(words []string) {
	...
	for chancesLeft > 0 {
		...
		rightGuess := false
		rightGuess, words = getMaxPartition(c, words)
		fmt.Println(words)
		if !rightGuess {
			fmt.Printf("Sorry, there are no %c\n", c)
			chancesLeft--
		} else {
			fmt.Println("Congrats, Your guess is correct")
			outWord = commonCharacter(words, outWord, c)
			fmt.Println("outWord", outWord)
			// Have won if string doesn't contain anymore _
			if !stringContains('_', outWord) {
				fmt.Println("You have won")
				fmt.Println("Correct word is " + words[0])
				return
			}
		}
	}
